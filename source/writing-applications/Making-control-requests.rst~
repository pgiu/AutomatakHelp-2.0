...
This interface has a method signature for every control and operating mode pair. In C++ you provide a callback lambda for the result, and in C# and Java a **_future_** of type CommandStatus is returned. A future is an interface that represents the eventual value of the operation. You may either _synchronously block_ for the result of the operation or _asynchronously listen_. The C# example below issues a CROB using the SBO operation mode and then calls Await() on the future to block the current thread until the control requests is complete:
